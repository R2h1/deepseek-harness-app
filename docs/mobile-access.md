...
引擎基于 cordis，是"一切皆插件"（web 应用由 patch 层叠加：模板 bundle → profile 的
`cordis.patch.yml` → `$DSH_HOME/cordis.patch.yml` → `--patch` 覆盖；装插件 =
`dsh plugin --profile web add <包>`，跑在**引擎进程内部**、用引擎内部 API）。

但"手机访问"选**壳**，不选插件：

| 维度 | 引擎插件 | 桌面壳（本方案） |
|---|---|---|
| 安全边界 | 网络暴露代码跑在**能执行 agent 代码的引擎进程内**——代理有漏洞即代码执行环境内 | 网络面与执行面**隔离**（纵深防御） |
| 引擎升级 | 依赖引擎内部 API，升级易碎（dsh-pocket 需补丁 cordis 即信号） | 只依赖引擎公开面（loopback HTTP 端点），升级不破 |
| 能力边界 | cordis 插件做不了系统托盘、原生窗口、进程生命周期——那是壳的领域 | 壳全能力 |
| 发版/测试 | 插件独立版本，需与引擎版本矩阵对齐 | 随应用发版，一套测试 |
| 实现成本 | 插件 + 壳接线，实质两块 | 一块 |

**不用第三方插件（dsh-pocket）的理由**：
1. 功能演进依赖他人维护/路线图；
2. 第三方代码跑在引擎（可执行代码）进程内 + GPL-2.0 许可证传染 = 供应链 + 许可 + 攻击面三重风险；
3. 自研放壳 = 自主可控 + 安全隔离，且不依赖任何第三方。

> 注：这套栈本来就有两个扩展层——引擎 cordis 插件（面向纯 CLI 用户扩展 Harness 能力）与
> 桌面壳（面向桌面用户做 OS 级能力）。"手机访问"是网络暴露 + 托盘/窗口 UI + 进程管理，
> 属于壳的领域，不是引擎 UX/agent 能力的扩展。选择壳不违背"一切皆插件"，而是选对了层。
